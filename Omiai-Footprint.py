# main.py  (Python 3.11 / 2nd gen Cloud Functions)
# Entry point: omiai_engine
import json, re, os
from flask import make_response, Request

# -------------------------------
# ライセンス許可リスト（無作為に生成した例）
# 必要に応じて増減・差し替えOK
# -------------------------------
ALLOWLIST = {
    "OMIAI-PRO-7K9J-2GXC-8Q43",
    "OMIAI-PRO-AV4M-U3T9-W7QK",
    "OMIAI-PRO-L2QH-9M5C-2PXD",
    "OMIAI-PRO-TH8V-1ZQK-7M3R",
    "OMIAI-PRO-G5XN-4JQ9-V2LH",
    "OMIAI-PRO-P3VK-8D2H-Q5ZM",
    "OMIAI-PRO-K9QT-7A4V-X2JN",
    "OMIAI-PRO-2RHF-6VY9-J4QM",
    "OMIAI-PRO-W8ZM-3QT2-L7HX",
    "OMIAI-PRO-J4NV-6M1Q-R8ZK",
    "OMIAI-PRO-X7QK-5HZ2-M9LV",
    "OMIAI-PRO-Q6LM-2VX9-H5RJ",
    "OMIAI-PRO-Z9TH-7Q2M-L4VK",
    "OMIAI-PRO-V2KM-8LQ4-H7ZX",
    "OMIAI-PRO-H5QZ-1V9M-K8TL",
    "OMIAI-PRO-M8LV-3Q5H-Z2KT",
    "OMIAI-PRO-R7QH-4M2V-X9JL",
    "OMIAI-PRO-T6ZL-9Q1H-V3KM",
    "OMIAI-PRO-L4HX-7V3Q-M2ZN",
    "OMIAI-PRO-N3QK-5Z8H-L7VM",
}

# -------------------------------
# ここに TRIAL/PRO の実体コードを埋め込み
# ※ どちらも (function(){ ... })(); の IIFE であること
# -------------------------------
TRIAL_ENGINE_JS = r"""
// --- Omiai Footprint Engine (Trial, 5 limit) ---
(function(){
  'use strict';
  const CONFIG = { IS_TRIAL:true, MAX_COUNT:5, BASE_WAIT_MAX:6, BETWEEN_WAIT_MS:900 };
  const sleep=(ms)=>new Promise(r=>setTimeout(r,ms));
  const randInt=(a,b)=>Math.floor(Math.random()*(b-a+1))+a;
  const CARD='.Profile__ProfileBox-sc-14hjqgs-0';
  const LIST='.ResultList__StyledDiv-sc-15q2fqo-0';
  const IMG=/profile_photo\/(\d+)\//i;
  const qsa=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const getCards=()=>qsa(CARD);
  const extractId=(c)=>{ const i=c.querySelector('img[src*="profile_photo/"]'); if(!i) return null; const m=(i.getAttribute('src')||'').match(IMG); return m&&m[1]?m[1]:null; };
  const onList=()=>!!document.querySelector(CARD);
  const onProf=()=>/\/profile(?:\/|\?|$)/.test(location.pathname)||(!document.querySelector(LIST)&&!document.querySelector(CARD));
  const dispatchSafe=(el,t,init)=>{ try{ (t.startsWith('pointer')?PointerEvent:MouseEvent); el.dispatchEvent(new (t.startsWith('pointer')?PointerEvent:MouseEvent)(t,init)); }catch{} };
  const clickInside=(el)=>{const c=el.closest('button,a,[role="button"]')||el.querySelector('img,button,a,[role="button"]')||el; const r=c.getBoundingClientRect(); const cx=Math.floor(r.left+r.width/2),cy=Math.floor(r.top+Math.min(24,r.height/2)); const base={bubbles:true,cancelable:true,composed:true,clientX:cx,clientY:cy}; dispatchSafe(c,'pointerdown',base); dispatchSafe(c,'mousedown',base); dispatchSafe(c,'mouseup',base); dispatchSafe(c,'click',base); try{typeof c.click==='function'&&c.click();}catch{} };
  const waitFor=async (cond,tm=12000,it=150)=>{const end=Date.now()+tm; while(Date.now()<end){ try{ if(await cond()) return true; }catch{} await sleep(it);} return false;};
  const isScrollable=(el)=>{ if(!el) return false; const st=getComputedStyle(el),oy=st.overflowY; return (oy==='auto'||oy==='scroll') && (el.scrollHeight-el.clientHeight>8); };
  const collect=()=>{ const hints=[document.querySelector('#wrapBox'),document.querySelector('.WrapBox__StyledWrapBox'),document.querySelector(LIST),document.querySelector('main'),document.getElementById('__next')].filter(Boolean); const arr=[]; hints.forEach(e=>{if(isScrollable(e))arr.push(e)}); qsa('div,main,section,article').forEach(e=>{if(isScrollable(e))arr.push(e)}); const u=Array.from(new Set(arr)); u.sort((a,b)=>(b.scrollHeight-b.clientHeight)-(a.scrollHeight-a.clientHeight)); return u; };
  const winT=()=>({type:'window', top(){return document.scrollingElement?document.scrollingElement.scrollTop:pageYOffset;}, scroll(dy){window.scrollBy(0,dy);}});
  const makeT=(el)=>({type:'element', top(){return el.scrollTop;}, scroll(dy){el.scrollBy(0,dy);}});
  let TGT=null, LISTS=[];
  const pick=()=>{ LISTS=[winT(), ...collect().map(makeT)]; TGT=LISTS[0]; };
  const scrollSmart=async(dy)=>{ for(const t of LISTS){ const b=t.top(); t.scroll(dy); await sleep(80); const a=t.top(); if(Math.abs(a-b)>=Math.min(8,Math.abs(dy)*0.2)){ TGT=t; return true; } } pick(); const t=TGT, b=t.top(); t.scroll(dy); await sleep(80); return Math.abs(t.top()-b)>=Math.min(8,Math.abs(dy)*0.2);};
  const autoMore=async()=>{ await scrollSmart(900); await sleep(400+Math.random()*300); };
  const nudgeLoad=async()=>{ for(let i=0;i<8;i++){ await scrollSmart(-200); await sleep(120+Math.random()*120); await scrollSmart(1200); await sleep(500+Math.random()*400); if(getCards().length>0) return true; } return false; };
  const done=new Set();
  const findNext=async()=>{ for(let t=0;t<12;t++){ const cards=getCards(); for(const c of cards){ const id=extractId(c); if(id && !done.has(id)) return {card:c,id}; } await autoMore(); } return null; };
  const randomProfileScroll=async()=>{ if(Math.random()<0.4){ await scrollSmart(300+Math.floor(Math.random()*500)); await sleep(200+Math.floor(Math.random()*300)); if(Math.random()<0.6){ await scrollSmart(-150-Math.floor(Math.random()*200)); await sleep(150+Math.floor(Math.random()*250)); } } };
  async function runTrial(){
    pick(); await sleep(200); pick();
    let processed=0, limit=CONFIG.MAX_COUNT;
    while(processed<limit){
      if(!onList()){ history.back(); if(!await waitFor(onList)) break; }
      const next=await findNext(); if(!next){ const ok=await nudgeLoad(); if(!ok) break; continue; }
      const {card,id}=next; try{ card.scrollIntoView({block:'center'});}catch{} await sleep(80); clickInside(card);
      let moved=await waitFor(onProf,8000,150);
      if(!moved && id){ const o=location.origin, cand=[`${o}/profile/${id}`,`${o}/profile?id=${id}`,`${o}/profile`]; for(const u of cand){ try{ location.assign(u); moved=await waitFor(onProf,6000,150); if(moved) break; }catch{} } }
      const dwell=randInt(2,Math.max(2,CONFIG.BASE_WAIT_MAX)); await randomProfileScroll(); await sleep(dwell*1000);
      if(id) done.add(id); processed++;
      history.back(); if(!await waitFor(onList,12000,150)) break;
      await scrollSmart(260); await sleep(160);
      await sleep(CONFIG.BETWEEN_WAIT_MS + Math.floor(Math.random()*500));
    }
    console.log('[Trial] finished', processed,'/',limit);
    alert(`体験版の上限(${limit}件)に達しました。続きは製品版でご利用ください！`);
  }
  (function panel(){
    const id='fw-trial-panel'; if(document.getElementById(id)) return;
    const css=`#${id}{position:fixed;right:16px;bottom:16px;z-index:2147483647;background:rgba(0,0,0,.75);color:#fff;padding:10px 12px;border-radius:12px;font:12px/1.4 system-ui,-apple-system,sans-serif;box-shadow:0 8px 24px rgba(0,0,0,.35);}#${id} button{padding:6px 10px;border:0;border-radius:8px;cursor:pointer;background:#38bdf8;color:#000}#${id} small{opacity:.8}`;
    const st=document.createElement('style'); st.textContent=css; document.documentElement.appendChild(st);
    const box=document.createElement('div'); box.id=id; box.innerHTML=`<div style="margin-bottom:6px"><b>Footprint Trial</b> <small>(5件まで)</small></div><div style="display:flex;justify-content:flex-end"><button id="fw-trial-start">試してみる</button></div>`;
    document.documentElement.appendChild(box);
    box.querySelector('#fw-trial-start').addEventListener('click',()=>{ const btn=box.querySelector('#fw-trial-start'); btn.disabled=true; runTrial().finally(()=>btn.disabled=false); });
  })();
  console.log('[Trial Engine] ready');
})();
"""

# ---- ここに “本番エンジン” を IIFE で貼り付け（あなたの v1.5.1 コードを即時実行化） ----
# 例: 下はダイジェスト。実際はあなたの「Omiai Footprint Walker」本体を丸ごと貼り付けてください。
PRO_ENGINE_JS = r"""
/* Omiai Footprint Walker PRO (v1.5.1) */
(function () {
  'use strict';
...
def omiai_engine(request: Request):
    # CORS preflight
    if request.method == 'OPTIONS':
        resp = make_response('', 204)
        for k,v in _cors_headers().items(): resp.headers[k]=v
        return resp

    token = (request.args.get('token') or '').strip()
    plan = 'pro' if token in ALLOWLIST else 'trial'
    code = PRO_ENGINE_JS if plan == 'pro' else TRIAL_ENGINE_JS

    payload = {
        "ok": True,
        "plan": plan,
        "version": "1.5.1" if plan == 'pro' else "trial-1.0.0",
        "code": code,
        "meta": {"ttl": 86400}
    }
