...
(function () {
  ...
  // ===== 設定 =====
  const ENDPOINT =
    "https://omiai-footprint-android-435226602223.asia-northeast1.run.app"; // prod
    ...
    GM_xmlhttpRequest({
      ...
      onload: (res) => {
        if (res.status !== 200) {
          ...
          if (showAlerts) {
            alert(
              `Footprinter エンジン取得エラー: HTTP ${res.status}\nCloud Functions の公開設定や URL を確認してください。`,
            );
          }
          return;
        }
        ...
        if (!data || !data.ok || (!data.code && !data.code_url)) {
          ...
          if (showAlerts)
            alert("エンジン取得エラー（code が含まれていません）");
          return;
        }
        ...
        const run = (codeStr) => {
          ...
          if (!ok && showAlerts) {
            alert(
              "Footprinter エンジンの実行に失敗しました。コンソールログをご確認ください。",
            );
          }
        };
        ...
        if (data.code_url) {
          ...
          GM_xmlhttpRequest({
            ...
            ontimeout: () => {
              if (showAlerts)
                alert("エンジンコード取得タイムアウト（code_url）");
            },
          });
        } else {
          run(data.code);
  ...
  if (typeof GM_registerMenuCommand === "function") {
    GM_registerMenuCommand("Footprinter エンジン再取得", () =>
      fetchAndRun(true),
    );
  }
})();
